...
#define __LDG_PTR   "r"
#endif
//change to ld.global.lu.u32 for mark as evict first
static __device__ __forceinline__ unsigned int __load_noncache(unsigned int *ptr) {
	unsigned int ret;
	asm volatile ("ld.global.u32 %0, [%1];"  : "=r"(ret) : __LDG_PTR (ptr));
	return ret;
}


//change to ld.global.ca.u32 for cache all levels
static __device__ __forceinline__ unsigned int __load_supercache(unsigned int *ptr) {
	unsigned int ret;
	asm volatile ("ld.global.u32 %0, [%1];"  : "=r"(ret) : __LDG_PTR (ptr));
	return ret;
}
static __device__ __forceinline__ void __prefetch(unsigned int * ptr) {
...
__global__ void dominate(struct allocation * in1, unsigned int * val1, unsigned int count1, unsigned int * donework, unsigned int * atom_count) {
	__shared__ struct allocation alloc[3][1024];
	__shared__ unsigned int val[3][1024];
	int start_index = 0;
	if(threadIdx.x == 0) {
		alloc[0][0].a[0] =atomicAdd(atom_count,(blockDim.x/32));

	}
	const char laneid = threadIdx.x % 32;
	__syncthreads();
	start_index = alloc[0][0].a[0] + (threadIdx.x / 32);// each
	...
	if(start_index < count1) {
		my_alloc = in1[start_index];
		my_val = val1[start_index];
	}
	while(__shfl((int)my_val,0) == 0) {
		if(laneid == 0) {
			start_index = atomicAdd(atom_count,1);
		}
		start_index = __shfl(start_index,0);
		if(start_index+1 > count1) {
			goto finished_domination;
		}
		start_index += laneid;
		if(start_index < count1) {
			my_alloc = in1[start_index];
	...
	for(int x=0;x < 3;x++) {
		if(start_index < count1) {
			alloc[x][threadIdx.x] = in1[start_index];
			val[x][threadIdx.x] = val1[start_index];
		}
		start_index+=32;
	}
	/*
	 * my_val = start_index - 4*32
	 * val[0] = start_index - 3*32
	 * val[1] = start_index - 2*32
	 * val[2] = start_index - 1*32
	 */
	struct allocation test_against;
	test_against.a[0] =(unsigned int) __shfl((int)my_alloc.a[0],0);
	if(laneid < 2) {
		my_alloc.a[0] = __shfl((int)my_alloc.a[0],1);
	}
	while(start_index < count1) {
		;
	}
	...
	if(threadIdx.x == 0) {
		alloc[0][0].a[0] = (atomicAdd(donework,1) == gridDim.x-1);
	}
	__syncthreads();
	if(alloc[0][0].a[0] == 0) {
		return;
...
__global__ void join(struct allocation * in1, unsigned int * val1, unsigned int count1,
	...
	//dynamic shared memory allocation, stores first #shared_indexes from in2
	extern __shared__ struct allocation alloc[];
	//allocate the cache with the first #SHARED_CAHCE entries in in2, only cache the allocation
	for(int x = threadIdx.x;x < shared_indexes; x+=blockDim.x) {
		alloc[x] = in2[x];
	}
	__syncthreads();
	//each thread have one allocation it compares agains all entries in in2
	struct allocation my_alloc;
	const char laneid = threadIdx.x % 32;
	// a mask used to find the threads lower than itself when using ballot
	...
	while (true) {
		int index;
		if (laneid == 0) {
			//get the next index we should fetch from in1
			index = atomicAdd(block_aloc, WARP_SIZE);
		}
		//get index from laneid 0
		index = __shfl(index, 0);
		//if we have checked all entries exit
		if(index >= count1) {
 			break;
		}
		int t_index = index + laneid;
		//get the value and allocation from in1
		if (t_index < count1) {
			my_alloc.a[0] = __load_noncache(&in1[t_index].a[0]);
			my_val = __load_noncache(&val1[t_index]);
		} else {
			//else set to conflict and value to zero
			my_alloc.a[0] = UINT_MAX;
			my_val = 0;
		}

		//here we fetch 32 allocation from in2 and compare it to all the threads allocation in in1
		for (int x = 0; x < count2; x +=32) {
			struct allocation tmp;
			unsigned int t_val = 0;
			if (laneid + x < count2) {
				t_val = val2[laneid + x];//val[cache_index][threadIdx.x];
				//if the allocation is in the cache
				if(laneid +x < shared_indexes) {
					tmp = alloc[laneid+x];
				} else {
					//else fetch from global memory
					tmp = in2[laneid+x];
				}
			} else {
				//if a thread is outside the bound of in2, set to conflict
				tmp.a[0] = UINT_MAX;
			}
			//here we query if which threads have an allocation from in2
			unsigned int available = __ballot((laneid + x) < count2);
			//if no thread have an allocation, we break, but it should not happen, I think we can remove it
			if (!available) {
				//allocate = 0;
				continue;
			}

			//now we check in1 vs in2, each thread have a in1 and in2 value
			for (int src_lane = 0; (available); src_lane+=2, available >>=2) {
				//we do two in parallel for performance, we could do more, thus 2 put variables
				struct allocation put;
				struct allocation put2;
				put.a[0] = 0;
				put2.a[0] = 0;
				//this loops just fast forwards until we have at least one thread with a compatible allocation
				while((__ballot(__shfl((int) tmp.a[0], src_lane) & my_alloc.a[0]) == 0) == 0 && available) {
					src_lane++;
					available >>= 1;
				}
				if(!available) {
				...
				//query the status if we have a compatible allocation amoung any thread in the warp
				unsigned int status1 = __ballot(put.a[0]);
				unsigned int status2 = __ballot(put2.a[0]);
				// if no, then just continue with the next, dont really need it as the while loop above ensures we have a
				...
				//now see we add it to the global memory
				if (status1) {
					int left = 0;
					if (laneid == 0) {
						//find how many compatible bids there are
						left = __popc(status1);
						//get the index we put in out
						put_index = atomicAdd(atom_count, left);
					}
					//give index to rest of threads
					put_index = __shfl(put_index, 0);
					if (put.a[0]) {
						//count how many preceding threads have allocation
			...
			//not sure it helps
			asm volatile ("prefetch.global.L2 [%0];" :: "l"(&in2[laneid + x + 32]));
			asm volatile ("prefetch.global.L2 [%0];" :: "l"(&val2[laneid + x + 32]));
		}
...
void test_run(void) {
	...
	struct allocation * out; unsigned int * outval;
	// the number of bids in each bin, make so the bids1 >= bids2 for scaling
	const unsigned int bids1 = 20000;
	const unsigned int bids2 = 10000;

	struct allocation * allocs1 = (struct allocation *) malloc(sizeof(struct allocation)*bids1);
	struct allocation * allocs2 = (struct allocation *) malloc(sizeof(struct allocation)*bids2);
	unsigned int * values = (unsigned int *) malloc(sizeof(unsigned int)*bids1);
	//if you gona run more than memory, put correct value here
	unsigned int max_bids = ((bids1+1)*(bids2+1)-1);
	unsigned int * rolling_index;
	printf("Setting up memory, max bids %u, tot amount %u\n",max_bids,bids1*bids2);
	...
	//change 2 to 1 for incompatible bids
	for(x=0;x<bids2;x++) {
		allocs2[x].a[0] = 2;
	}
...
//	CUDA_CHECK_RETURN( cudaMemset(val2,1, sizeof(unsigned int)*bids));
	...
	//number of blocks, if SHARED_CACHE is 12288, we can only have 5 blocks
	// half SHARED_CACHE if you want 10 blocks
	int blocks = 1;
	...
	int threads = 1024;
	//change this to 0 to fit as many blocks in the gpu, or 1 for maximum cache with maximum of 5 blocks
	int full_cache =1;
	const int max_memory = 48*1024; //bytes in shared memory
	int n_cache_entries;
	if(full_cache) {
		n_cache_entries = max_memory/sizeof(struct allocation);
	} else {
		//scale the memory according to the number of threads
		//for full utilization, 24 bytes is the maximum amount of shared memory you can have per thread
		// 48k/1024 = 48 bytes, we have to have half to allow maximum utilization thus 24.
		const int bytes_p_thread = 24;
		int entries = bytes_p_thread/sizeof(struct allocation);
		n_cache_entries = threads*entries;
	}
	printf("Running kernel, with shared memory %d entries totaling %lu bytes\n",n_cache_entries,n_cache_entries*sizeof(struct allocation));
	join <<<blocks, THREADS,sizeof(struct allocation) * n_cache_entries,stream1>>>(
			in1,val1,bids1,
			in2,val2,bids2,
			out,outval,

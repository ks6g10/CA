...
def lpex2(filename, method):
    ...
    path = os.path.realpath(filename).split(os.path.basename(filename))[0]+"cplex/"
    print "Path to the result", path
    new_file = path+os.path.basename(filename).split(".lp")[0] + ".cplex"
    print "File of the result", new_file
    #filename =  os.path.basename(filename)
    #print os.getcwd()


    
    #filename = os.getcwd()+'/' + filename.strip()
    
    #print filename
    
    c = cplex.Cplex(str(filename))
    c.parameters.tuning.timelimit.set(3600.0)
    ...
    f.write("VAL : " + str(c.solution.get_objective_value()) + "\n")
    f.close()
